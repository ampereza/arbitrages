...
import { TokenFetcher, Token } from './TokenFetcher.js';
...
class ArbitrageBot {
    ...
    private async getUniV2Price(router: string, dexName: string, amountIn: bigint, path: string[], decimalsOut: number): Promise<number> {
        try {
    ...
    private async getUniV3Price(amountIn: bigint, tokenIn: string, tokenOut: string, decimalsOut: number): Promise<number> {
        try {
    ...
    private async get1InchPrice(amountIn: bigint, fromToken: string, toToken: string, decimalsOut: number): Promise<number> {
        try {
    ...
    private async getZeroXPrice(amountIn: bigint, sellToken: string, buyToken: string, decimalsOut: number): Promise<number> {
        try {
    ...
    private async checkArbitrage(prices: PriceQuote[], baseToken: Token, quoteToken: Token) {
        // Use comprehensive analysis instead of manual calculation
        try {
            const analysis = await this.tokenFetcher.analyzeArbitrageOpportunity(
                baseToken,
                quoteToken,
    ...
    private estimatePoolLiquidity(dexName: string, token: Token): number {
        ...
        const dexLiquidityEstimates: { [key: string]: number } = {
    ...
    private getTokenLiquidityMultiplier(tokenSymbol: string): number {
    ...
    private getDexPoolType(dexName: string): 'uni-v2' | 'uni-v3' | 'sushi' | 'aave' {
    ...
    private calculateOptimalTradeSize(tokenPrice: number, tokenSymbol: string): number {
    ...
    private getTargetTradeValue(tokenSymbol: string): number {
    ...
    private getMinimumTradeSize(tokenSymbol: string): number {
    ...
    private estimateGasCosts(): number {
    ...
    private getTestAmount(token: Token): bigint {
        ...
        try {
            ...
            while (this.isRunning) {
                for (const pair of tradingPairs) {
                    ...
                    const zeroXPrice = await this.getZeroXPrice(
                        testAmount,
